					...
					<view v-for="(item,rowIndex_2) in currentItem.QuestionOptionList" v-bind:key='rowIndex_2'>
						<view class="exam-item-option">
							<checkbox :value="setCheckboxVal(item.fldOptionIndex)"
								:checked="checkboxCheck(item.fldOptionIndex)" />
							<text>{{checkNum(item.fldOptionIndex)}}</text>.<text>{{item.fldOptionText}}</text>
						</view>
					</view>
				</checkbox-group>
				...
				<view class="exam-button-row">
					<view>
						...
						<u-button v-show="currentIndex != 0" size='mini' @click="lastQuestion"
							style="margin-right: 20rpx; border-right: 1rpx;">{{lastText}}</u-button>

						<u-button v-show="currentIndex < dataList.length-1 " size='mini' @click="nextQuestion">
							{{nextText}}
						</u-button>

						<u-button size='mini' @click="finish" style="margin-right: 20rpx; margin-left: 20rpx;">
							{{finishText}}
						</u-button>
						
						<!-- 点击完成后,弹窗提醒↓ -->
						<u-modal v-model="show" @confirm="confirm" :content="content" :show-cancel-button="true"></u-modal>
	...
	export default {
		...
		methods: {
			...
			finish() { //点击完成按钮
				...
				// });
				this.show = true
				//console.log(this.dataList); //打印附带作答情况的dataList			
			},
