...
<script>
	export default {
		...
		methods: {
			GetData(){
				this.$Request.getRequest(this.$url.GATHER.specificCourse, this.$global.vxUser.courseId).then(res => {
					if(res.code==-1){
						uni.showToast({
							icon: 'none',
							title: '获取课程信息失败',
						...
						this.nameClass=res.result[0].cou_name
						this.introduceClass=res.result[0].cou_synopsis
						this.imgList.push(res.result[0].cou_picture)
						this.picture=this.imgList[0]
						this.school=res.result[0].cou_school
						this.GetExitTimeIndex(res.result[0].cou_existtime)
					}
				}).catch(err => {
					console.log('出错了，错误是', err);
				})
			},
			GetExitTimeIndex(exitTime){
				// 获取结课时间对应索引,并创建结课数组
				this.picker = []
				let t = new Date()
				...
				for (let i = 0; i < 4; i++) {
					...
					if(exitTime==((this.tempDate + i) + "-上")){
						this.index=2*i
					}else if(exitTime==((this.tempDate + i) + "-下")){
						this.index=2*i+1
					}
				}
			},
